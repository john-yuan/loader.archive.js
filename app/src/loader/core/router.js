...
define(function (require, exports, module) {
    ...
    var QS = require('loader/utils/queryString');
    ...
    var getViewParams = function () {
        ...
        return QS.decodeQueryString(queryString, hooks.processQsValue);
    };
    ...
    var routeTo = function (viewName, viewParams, extraData) {
        ...
        // 判断是否有参数
        if (viewParams) {
            viewParams = QS.encodeQueryString(viewParams);
        ...
        if (typeof DEBUG !== 'undefined' && DEBUG === true) {
            if (hash.length > 2000) {
                utils.error('检测到 hash 长度超过 2000，请适当减少 URL 参数数量，或使用 extraData 进行数据传递。');
            }
        }
    ...
    var onHashChange = function () {
        ...
        if (typeof DEBUG !== 'undefined' && DEBUG === true) {
            utils.info('检测到 hash 变化。hash=' + decodeURIComponent(hash));
        }
